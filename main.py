...
def get_access_token():
    ...
    try:
        ...
        raise Exception(f"⚠️ Token request failed: {e}")
...
def initialize_prev_values():
    try:
        ...
        if res.get("s") == "ok" and 'd' in res:
            ...
            print("✅ Initialized previous values")
    except Exception as e:
...
def force_fetch(symbol: str):
    ...
    try:
        ...
        fyers = fyersModel.FyersModel(client_id=CLIENT_ID, token=ACCESS_TOKEN)
        try:
            res = fyers.quotes({"symbols": f"NSE:{sym}-EQ"})
        except Exception as e:
            return {"error": f"Fyers quotes API failed: {e}"}

        now_ts = time.time()
        ltp = volume = delta_qty = buy_vol = sell_vol = 0
        if res.get("s") == "ok" and "d" in res:
            data = res["d"][0]["v"]
            ltp = data.get('lp', 0) or data.get('ltp', 0)
            volume = data.get('volume', 0)
        ...
        cache_expiry[sym] = now_ts
        return latest_data[sym]

    except Exception as e:
        return {"error": f"Force fetch failed: {e}"}
